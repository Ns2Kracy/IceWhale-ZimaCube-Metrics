...
import (
	...
	"github.com/Ns2Kracy/IceWhale-ZimaCube-Metrics/model"
	"gopkg.in/ini.v1"
)

var (
	ZimaCubeMetricsConfigFilePath = filepath.Join(constants.DefaultConfigPath, "zimacube-metrics.conf")
...
func InitSetup(config string, sample string) {
	ConfigFilePath = ZimaCubeMetricsConfigFilePath
	if len(config) > 0 {
